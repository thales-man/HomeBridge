...
final class BridgedOutlet: HAP.Accessory.Outlet, BridgedAccessory {
    ...
    override func characteristic<T>(_ characteristic: GenericCharacteristic<T>,
        ...
        if characteristic === outlet.powerState {
            let requestedChange = outlet.powerState.value!
    ...
    func switchState(requestedState: String) {
        outlet.powerState.value = deviceIsActive(requestedState)
    }
}
